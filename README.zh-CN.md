...
4. ink-renderer 输入框出现粘贴片段标签（如 `[pasted src/foo.ts:12-14 · 3 lines]`），确认后提交即可
...
| ink-renderer | `@agenteam/ink-renderer` + 宿主实现 IDE Bridge（`ctl-command/ink-renderer.ts` 中的 `observeSnippets`） |
...
- 更新宿主代码后需重启 `agenteam`，确保已接线 `observeSnippets`

**发送成功但 TUI 无反应**

- 宿主需实现 `observeSnippets` + `startIdeBridgeSubscriber`（IDE Bridge v0.2.1+）
...
- lock 心跳（每 15s 刷新 `updatedAt`），便于 ink-renderer 发现
